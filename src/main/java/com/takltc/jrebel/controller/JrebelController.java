package com.takltc.jrebel.controller;

import com.takltc.jrebel.config.JrebelConfigurationProperties;
import com.takltc.jrebel.sys.JrebelSign;
import com.takltc.jrebel.sys.rsasign;
...
public class JrebelController {
    ...
    public void jrebelLeases1Handler(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // Parameters sent by the IDE/agent on the seat-update request
        ...
        if (offline) {
            ...
            long clientTimeUntil = 0L;
            if (clientTime != null) {
                // 180 days validity window, same as in the initial lease handler
                clientTimeUntil = Long.parseLong(clientTime) + 180L * 24 * 60 * 60 * 1000;
        ...
        // Compute signature when all required parameters are present
        if (clientRandomness != null && guid != null) {
            JrebelSign sign = new JrebelSign();
            sign.toLeaseCreateJson(clientRandomness, guid, offline, validFrom, validUntil);
            jsonObject.put("signature", sign.getSignature());
        }
